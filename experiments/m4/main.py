...
class M4Experiment(Experiment):
    ...
    def instance(
        ...
        for seasonal_pattern in M4Meta.seasonal_patterns:
            ...
            # Training Set
            training_values = group_values(
                dataset.values, dataset.groups, seasonal_pattern
            )
            ...
            snapshot_manager = SnapshotManager(
                snapshot_dir=os.path.join(self.root, "snapshots", seasonal_pattern),
        ...
        forecasts_df = pd.DataFrame(
            forecasts, columns=[f"V{i + 1}" for i in range(np.max(M4Meta.horizons))]
        )
