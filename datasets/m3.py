...
class M3Dataset:
    ...
    def load(training: bool = True) -> "M3Dataset":
        values_file = (
            TRAINING_SET_CACHE_FILE_PATH if training else TEST_SET_CACHE_FILE_PATH
        )
        return M3Dataset(
            ids=np.load(IDS_CACHE_FILE_PATH, allow_pickle=True),
    ...
    def to_hp_search_training_subset(self):
        return M3Dataset(
            ...
            horizons=self.horizons,
            values=np.array(
                [v[: -2 * self.horizons[i]] for i, v in enumerate(self.values)]
            ),
        )
    ...
    def download() -> None:
        ...
        for sp in M3Meta.seasonal_patterns:
            ...
            training_values.extend(
                [
                    ts[~np.isnan(ts)][:-horizon]
                    for ts in dataset[dataset.columns[6:]].values
                ]
            )
            test_values.extend(
                [
